App.service('util_SERVICE', ['$http', '$window', '$cookieStore', '$rootScope', function ($http, $window, $cookie, $rootScope) {
	...
	this.info = function(title,bodytext,type)
	{
		$rootScope.infoheader = title;
		
		$rootScope.infobody = bodytext;
...
//Get All company list
    this.GetAllCompany = function () {
        var data = {"apikey" : this.APIKEY,'method':"AdminGetAllCompany"}
...
//Get All Department list
    this.GetAllDepartment = function () {
        var data = {"apikey" : this.APIKEY,'method':"AdminGetDepartment"}
        var promise = $http.post(this.url, "sJsonInput=" +JSON.stringify(data), this.config)
   .success(function (response) {
       if (response.returnStatus == 1) {
           return response;
